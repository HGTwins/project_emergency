...
export default function AdminPage() {
    ...
    const fetchMembers = async (page: number = 0) => {
        ...
        if (!headers) return;
        try {
            const resp = await fetch(`/proxy/api/admin/getMembers?page=${page}&size=8`, { headers });
            if (resp.ok) {
    ...
    const fetchAllReviews = async (page: number = 0) => {
        ...
        if (!headers) return;
        try {
            const resp = await fetch(`/proxy/api/review?page=${page}&size=8`, { headers });
            if (resp.ok) {
                const data = await resp.json();
    ...
    const deleteMember = async (username: string) => {
        ...
        const headers = getHeaders();
        const resp = await fetch(`/proxy/api/admin/getMember/${username}`, { method: 'DELETE', headers: headers! });
    ...
    const deleteReview = async (seq: number) => {
        ...
        const headers = getHeaders();
        const resp = await fetch(`/proxy/api/review/${seq}`, { method: 'DELETE', headers: headers! });
        if (resp.ok) {
            alert("삭제되었습니다.");
